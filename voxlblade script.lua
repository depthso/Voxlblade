...
local function WalkTo(TargetCFrame: CFrame)
	local Character = LocalPlayer.Character
	local Humanoid = Character:FindFirstChildOfClass("Humanoid")
	
	local From = Character:GetPivot().Position
	local To = TargetCFrame.Position
	...
	while true do
		...
		if RetryCount > MaxRertries then return end
	...
	local function Process(Waypoint)
		if not Enabled or not AutoFarm.Value then
			return Stop(false) 
		end
	...
	local function Reached(HasReached: boolean)
		WaypointIndex += 1
		
		if not HasReached or WaypointIndex > #WayPoints then
			return Stop(true)
		end
	...
	local function Blocked(WaypointNumber: number)
		if WaypointNumber > WaypointIndex then
			--WalkTo(To, true)
			Stop(false)
		end
	end
	...
	--// Connect events
	ReachedConnection = Humanoid.MoveToFinished:Connect(Reached)
	BlockedConnection = Path.Blocked:Connect(Blocked)

	Process(WayPoints[WaypointIndex])
...
local function MoveToPoint(TargetCFrame: CFrame)
	...
	if Method == "Path find" then
		local Success = WalkTo(TargetCFrame)
		if Success then return end
	end
	
	if not Enabled or not AutoFarm.Value then return end
...
local function FindEmermy(Name: string, InFarmland: boolean?): BasePart?
	local Character = LocalPlayer.Character
	local RootPosition = Character:GetPivot().Position
